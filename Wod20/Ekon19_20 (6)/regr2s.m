...
Ldemp=130; Hammer=1; wspZ=0.02; 
[Temp] = readvars( filename );
Ldemp = length(Temp);
...
tx=x'; Ldanych=Ldemp; 
NT=(max(tx)-min(tx))/4; 
FIh(1:Ldanych,1)=1; i=2; 
for(k=1:4)
     FIh(1:Ldanych,i)=sin(2*pi*k*tx/NT);    i=i+1; 
     FIh(1:Ldanych,i)=cos(2*pi*k*tx/NT);    i=i+1; 
end
Y=2+10*FIh(:,2)+5*FIh(:,3)+10*FIh(:,4)+5*FIh(:,5)+10*FIh(:,6)+5*FIh(:,7)+10*FIh(:,8)+5*FIh(:,9);
figure(100); plot(tx,Y,'.-')
...
U=FI*P; % transformacja ortogonaliz. Karhunena-Loevego: U macierz wej�� ortogonalnych
s2=0; 
for(i=1:Kd) s2=s2+S(i,i); s(i)=S(i,i); end, % Udzia� mocy kolumn ortogonalnych skladowych 
...
while(dalej)
    ...
    for(k=1:Kf)
       if(k==1) 
           ...
           if(istot(k)<=0) 
               tSt(k)=0; % cz�on nieistotny - kandydat do usuni�cia
               if(k>Kd) Rfe(k)=0;
               else
