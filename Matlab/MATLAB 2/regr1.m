...
input(' Mamy dane empiryczne - <Ent> pokaz hipotetycznych zak³óceñ ')
...
xlabel(sprintf(' Dane empiryczne i hipotetyczne zak³ócenia %s_z=%.2f %s %s',txsig,wspZ,txalf,txbet));
input(' <Ent> liczymy regresjê ')
...
while(dalej)
    ...
    kus=input(sprintf('\nUsuniecie wejscia %d: tst=%.2f <Ent> lub wpisz inny numer wejœcia; 0 - KONIEC ?? ',kmin, tMale(mmin))); 
