...
Lsk=60;  Lkor=20;  
jednakowe=0;  
AR=0; %AR=1; % 0 - liczymy korelacjê wzjemn¹ Rrn; 1 liczymy autokorel.Rrr
...
Sgdr=sqrt(sum(sgdr(1,1:Lsk).^2)/(Lsk^2));  % odch.standard. œredniej 
a=0; %10; 
...
srdn=20; sigdn=100;  % srednia i odch.stand.
alfa=0.8; % Parametr dla AR1 (wsp. autoregresji= wspo³cz. autokorel. R1)
...
if(AR) Rteor=alfa.^[1:Lkor]; else Rteor=zeros(1,Lkor); end 
% teraz rysunki 
...
for(m=1:Ltau) 
    ...
    plot([1:Ldanych],R(tau(m),:),kol(m),[1 Ldanych],Rteor([1 Lkor]),[kol(m) ':']);  hold on; 
end; 
hold off; axis('tight'); 
xlabel(tx)
